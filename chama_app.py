...
@app.route('/admin/loans/update/<int:loan_id>', methods=['POST'])
@login_required
@admin_required
def update_loan_status(loan_id):
    ...
    if status in ['Approved', 'Rejected', 'Repaid']:
        loan.status = status
        db.session.commit()
        ...
        notification = Notification(
            title=f'Loan {status}',
            message=f'Your loan application for {loan.amount} has been {status.lower()}.',
            user_id=member.user_id
        )
        db.session.add(notification)
        db.session.commit()
        
        flash(f'Loan status updated to {status}', 'success')
    else:
